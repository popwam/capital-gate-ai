...
async function confirmCriticalMappings(item) { for (const issue of item.issues.filter((entry) => entry.field?.startsWith("column:") && entry.options?.suggestedValue && !entry.resolvedAt)) item = await resolve(item, issue.field, issue.options.suggestedValue); return item; }
...
async function main() {
  ...
  const login = await fetch(`${base}/admin/auth/login`, { method: "POST", headers: { Origin: "http://localhost:3000", "content-type": "application/json" }, body: JSON.stringify({ email, password }) }); assert.ok(login.ok, `login status=${login.status} body=${await login.text()}`); cookie = (login.headers.get("set-cookie") || "").split(";")[0];
  ...
  item = await confirmCriticalMappings(item);
  ...
  let multi = await upload(multiRows, `typed-multiple-${stamp}.xlsx`); multi = await resolve(multi, "projectId", projectId); multi = await resolve(multi, "currency", "EGP"); multi = await confirmCriticalMappings(multi);
