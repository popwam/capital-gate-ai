...
function Stat({label,value,sub}:{label:string;value:number;sub:string}){
  return <div className="rounded-[10px] border border-[#e0e4e1] bg-white p-3.5"><p className="text-[11px] font-bold text-[#5f6d67]">{label}</p><p className="mt-1.5 text-xl font-extrabold tracking-[-.03em]">{fmt.format(value||0)}</p><p className="mt-0.5 text-[11px] text-[#5f6d67]">{sub}</p></div>;
}
function Action({icon:Icon,label,value,href,warn=false}:{icon:typeof Warehouse;label:string;value:number;href:string;warn?:boolean}){
  return <a href={href} className="flex items-center gap-3 rounded-[10px] border bg-white p-3 transition hover:border-[#bac7c0]"><span className={`grid h-8 w-8 place-items-center rounded-lg ${warn&&value>0?"bg-[#f6ead7] text-[#8a672f]":"bg-[#edf2ef] text-[#35594d]"}`}><Icon size={16}/></span><span className="min-w-0 flex-1"><b className="block truncate text-[13px]">{label}</b><small className="text-[11px] text-[#5f6d67]">فتح ومراجعة</small></span><b className="text-sm">{fmt.format(value||0)}</b></a>;
}
...
export default function AdminDashboardPage(){
  ...
  return <main className="mx-auto max-w-[1420px] p-4 sm:p-6" dir="rtl">
    <div className="flex flex-wrap gap-2"><a href="/admin/data/import" className="min-h-10 rounded-lg bg-[#285f55] px-3 py-2.5 text-xs font-bold text-white">استيراد مخزون</a><a href="/admin/inventory" className="min-h-10 rounded-lg border bg-white px-3 py-2.5 text-xs font-bold">فتح المخزون</a><a href="/admin/projects" className="min-h-10 rounded-lg border bg-white px-3 py-2.5 text-xs font-bold">المشروعات</a></div>
    ...
    <div className="mt-4 overflow-x-auto rounded-[10px] border bg-white p-1"><div className="flex min-w-max gap-1">{tabs.map(([value,label])=><button key={value} onClick={()=>setTab(value)} className={`rounded-lg px-3 py-2 text-xs font-extrabold transition ${tab===value?"bg-[#285f55] text-white":"text-[#65736d] hover:bg-[#f2f4f1]"}`}>{label}</button>)}</div></div>

    <section className="mt-4 min-h-[300px]">
      {tab==="OVERVIEW"&&<div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]"><div className="overflow-hidden rounded-2xl border bg-white"><div className="flex items-center justify-between border-b p-4"><div><b>آخر الاستيرادات</b><p className="mt-1 text-xs text-[#5f6d67]">أربع دفعات فقط</p></div><a href="/admin/data" className="text-xs font-bold text-[#765b31]">عرض الكل</a></div><div className="divide-y">{imports.length?imports.map((batch)=><a key={batch.id} href={`/admin/data/import?import=${batch.id}`} className="grid gap-2 p-4 hover:bg-[#fafaf7] sm:grid-cols-[1fr_auto_auto] sm:items-center"><div className="min-w-0"><b className="block truncate text-sm" dir="auto">{batch.name||batch.fileName}</b><small className="text-[#5f6d67]">{fmt.format(batch.rowsDetected)} صف · {fmt.format(batch.rowsCreated)} جديد</small></div><small>{new Date(batch.uploadedAt).toLocaleDateString("ar-EG",{day:"numeric",month:"short"})}</small><span className="w-fit rounded-full bg-[#eef2ef] px-2.5 py-1 text-[11px] font-bold">{statusLabel(batch.status)}</span></a>):<p className="p-8 text-center text-sm text-[#5f6d67]">لا توجد دفعات.</p>}</div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><Action icon={FileSpreadsheet} label="استيراد يحتاج مراجعة" value={dashboard?.importsNeedingInput??0} href="/admin/data" warn/><Action icon={MessageSquareText} label="محادثات آخر 24 ساعة" value={dashboard?.conversations24h??0} href="/admin/conversations"/><Action icon={UsersRound} label="متابعات مستحقة" value={leadSummary?.followUpsDue??0} href="/admin/leads" warn/></div></div>}
      ...
      {tab==="AI"&&<div className="rounded-2xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><div><b>عقول Cg Ai</b><p className="mt-1 text-xs text-[#5f6d67]">الحالة الفعلية للمزودين والموديلات</p></div><span className={`grid h-11 w-11 place-items-center rounded-xl ${aiHealthy?"bg-[#eaf3ee] text-[#356c58]":"bg-[#f8ecdd] text-[#8a672f]"}`}>{aiHealthy?<CheckCircle2 size={20}/>:<AlertCircle size={20}/>}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{aiItems.length?aiItems.map((item,index)=><div key={`${item.provider}-${index}`} className="rounded-2xl border bg-[#fbfbf8] p-4"><div className="flex items-center gap-2"><Bot size={17}/><b>{item.provider||"AI"}</b><span className={`me-auto h-2.5 w-2.5 rounded-full ${item.healthy===false?"bg-red-500":"bg-emerald-500"}`}/></div><p className="mt-2 truncate text-xs text-[#5f6d67]" dir="auto">{item.model||item.selectedModel||"Dynamic route"}</p>{item.errorCode&&<p className="mt-2 text-xs text-red-700">{item.errorCode}</p>}</div>):<p className="text-sm text-[#5f6d67]">لم تصل بيانات الصحة بعد.</p>}</div><a href="/admin/system" className="mt-4 inline-flex rounded-xl border px-4 py-2.5 text-xs font-bold">تفاصيل النظام</a></div>}
