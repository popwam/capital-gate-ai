...
export class KnowledgeController {
  ...
  @Patch("items/:itemId") update(@Param("itemId") id: string, @Body() body: ItemDto) { return this.knowledge.updateItem(id, body.content, body.approvalStatus); }
  @Post(":knowledgeId/approve") async approve(@Param("knowledgeId") id: string, @Req() req: any) { const item = await this.knowledge.approve(id); await this.audit.record(req.admin.id, "KNOWLEDGE_APPROVED", "ProjectKnowledge", id); return item; }
}
