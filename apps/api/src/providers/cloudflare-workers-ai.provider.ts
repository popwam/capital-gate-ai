...
export class CloudflareWorkersAIProvider implements AIProvider {
  ...
  async extractIntent(messages: AIMessage[], previous: StructuredIntent) {
    const raw = await this.run(this.fastModel, this.structuredPrompt(
      "Extract and update Egyptian real-estate conversation state. Understand Egyptian Arabic, MSA, English, mixed Arabic/English, and Arabizi. New explicit corrections replace conflicting current preferences. Keep historical rejections separately. Extract language, dialect, purpose, locations, rejectedLocations, propertyTypes, bedrooms, bathrooms, budgetMin, budgetMax, budgetFlexible, currency, deliveryMaxYears, maxDownPayment, maxTravelMinutes, minimumArea, maximumArea, hardRequirements, softPreferences, requestedMedia, requestedProject, exactRouteRequested, routeOrigin, routeDestination, purchaseIntent 0-100, contactName, contactPhone, rejectedProjects, preferredDevelopers, preferredProjects, familyRequirements, investmentRequirements, customerConcerns.",
