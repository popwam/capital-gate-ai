...
export class LocationsController {
  ...
  @Post("distances") async distance(@Body() body: DistanceDto, @Req() req: any) { const item = await this.prisma.$transaction(async tx => { const first = await tx.locationDistance.upsert({ where: { fromLocationId_toLocationId: { fromLocationId: body.fromLocationId, toLocationId: body.toLocationId } }, create: { ...body, distanceType: body.distanceType || "ADMIN_VERIFIED", verifiedAt: new Date() }, update: { ...body, verifiedAt: new Date() } }); if (body.isBidirectional !== false) await tx.locationDistance.upsert({ where: { fromLocationId_toLocationId: { fromLocationId: body.toLocationId, toLocationId: body.fromLocationId } }, create: { ...body, fromLocationId: body.toLocationId, toLocationId: body.fromLocationId, distanceType: body.distanceType || "ADMIN_VERIFIED", verifiedAt: new Date() }, update: { distanceKm: body.distanceKm, estimatedMinutes: body.estimatedMinutes, notes: body.notes, verifiedAt: new Date() } }); return first; }); await this.audit.record(this.admin(req), "LOCATION_DISTANCE_SAVED", "LocationDistance", item.id); return item; }
}
