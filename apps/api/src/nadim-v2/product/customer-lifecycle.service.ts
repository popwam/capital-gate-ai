...
export class CustomerLifecycleService {
  ...
  async saveRequirement(input: { conversationId: string; channel: NadimChannel; externalUserId?: string; state: NadimState; title?: string; status?: PropertyRequirementStatus; allowNew?: boolean }) {
    ...
    const data = {
      ...
      budgetOriginalAmount: search.budget?.originalAmount,
      budgetOriginalCurrency: search.budget?.originalCurrency,
      budgetNormalizedAmount: search.budget?.normalizedAmount,
      budgetNormalizedCurrency: search.budget?.normalizedCurrency,
      fxRate: search.budget?.fxRate,
      fxAsOf: search.budget?.fxAsOf ? new Date(search.budget.fxAsOf) : undefined,
      fxSource: search.budget?.fxSource,
