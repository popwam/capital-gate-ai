...
function explicitUnderstanding(message: string, state: NadimState): NadimUnderstanding {
  ...
  const bedroom = text.match(/(?:خليها\s*)?(\d{1,2})\s*(?:غرف(?:ة)?|bed(?:room)?s?)/iu)
    ?? text.match(/(?:غرف(?:ة)?|bed(?:room)?s?)\s*(\d{1,2})/iu);
  ...
  return {
    intent,
    confidence: intent === "UNKNOWN" ? 0.35 : 0.9,
    locale: /[\u0600-\u06FF]/u.test(text) ? "ar-EG" : "en",
    operations,
