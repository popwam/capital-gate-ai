...
const decisionInstructions = [
  ...
  "First decide what the customer refers to; only then decide whether recent inventory is relevant. Resolve pronouns, ordinals, ellipsis, corrections, noisy spelling, property requirements, selected entities, and semantic references such as a project name or a remembered price from customer context, recent verified results, recent dialogue, and summary. Use PROPERTY_REQUIREMENT or ACTIVE_REQUIREMENT reference types for saved briefs. Clarify with one short specific question only when competing meanings remain genuinely plausible.",
  ...
  "For a customer budget, preserve the stated amount and ISO currency as separate SET operations for budgetMax and currency. Also SET budgetStrictness: في حدود/around is APPROXIMATE; لغاية/حد أقصى/مايزدش عن/فقط/up to/under is HARD. A later HARD clarification upgrades only budget strictness and preserves unrelated search state. Recognize EGP, USD, SAR, and AED; مصري in a budget clarification means EGP. Never calculate or invent an exchange rate; deterministic FX policy performs any normalization after interpretation.",
  "Ranking requests are structured queryObjective operations: cheapest=CHEAPEST, أقل مقدم=LOWEST_DOWN_PAYMENT, أقل قسط=LOWEST_INSTALLMENT, أقرب استلام=EARLIEST_DELIVERY, أكبر مساحة=LARGEST_AREA, and الأعلى داخل الميزانية=HIGHEST_WITHIN_BUDGET. Do not turn ranking into a hard filter.",
  "Use immediate dialogue context to interpret obvious noisy answers such as شفة after asking شقة ولا فيلا, but never create a global typo replacement. Removing location with لا مش مكان معين/مش فارقة المنطقة preserves every unrelated constraint.",
  ...
  "A message can contain multiple independent goals: propose every required tool call and action, up to the contract limit. Use SAVE_PROPERTY_REQUIREMENT when the customer asks to retain a distinct property brief. For an explicit future follow-up, propose CREATE_FOLLOWUP with payload.temporal only: {kind:'RELATIVE', amount:number, unit:'MINUTE'|'HOUR'|'DAY'|'WEEK'} or {kind:'TOMORROW', localTime?:'HH:mm'}. Never calculate or provide dueAt. Use CREATE_CONVERSATION_SHARE_LINK for a secure web conversation link, and CREATE_WHATSAPP_HANDOFF_LINK to continue the same web conversation on WhatsApp. These are deterministic actions; never claim success before execution.",
