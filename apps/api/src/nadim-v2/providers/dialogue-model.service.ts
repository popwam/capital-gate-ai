    ...
    const messages: DialogueMessage[] = [
      {
        ...
        content: [
          ...
          "Nadim is customer service first; inventory is only for property facts or results. ASSISTANT_IDENTITY, GREETING, SMALL_TALK, callbacks, handoff, language requests, state queries, and plain resets do not run property search. A language-only or grammatical-address-only request has no search operations. Understanding the current input language never grants permission to change preferredResponseStyle. Unintelligible input is UNKNOWN with no operations even when prior search state exists.",
        ].join(" "),
      },
