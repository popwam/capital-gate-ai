...
export function exactExternalUnitId(source: string) {
  const match = source.match(/(?:unit|الوحده|وحده)?\s*([a-z][a-z0-9-]*\s+\d+\s*\/\s*\d+)\b/iu);
...
export function planCustomerTurn(source: string, previous: StructuredIntent): TurnPlan {
  ...
  if (/(?:وريني|هات|اعرض|ابعت).*(?:الصور|صور)|(?:photos?|images?)\b/iu.test(text))
    return { intent: "MEDIA_REQUEST", requiresDatabase: true, requiresExtraction: false, emitCards: false, executeBrochure: false };
  ...
  if (/(?:نظام\s+السداد|خطه\s+السداد|تقسيط|payment plan|installments?)/iu.test(text))
    return { intent: "PAYMENT_PLAN", requiresDatabase: true, requiresExtraction: true, emitCards: false, executeBrochure: false };
  ...
  if (unitId) return { intent: "PROPERTY_DETAILS", requiresDatabase: true, requiresExtraction: false, emitCards: false, executeBrochure: false, exactUnitId: unitId };
...
export function applyDeterministicTurnSemantics(source: string, extracted: StructuredIntent, previous: StructuredIntent, plan: TurnPlan): StructuredIntent {
  ...
  if (plan.exactUnitId) {
    ...
    // Identifier fragments such as `4/2` are never room counts.
